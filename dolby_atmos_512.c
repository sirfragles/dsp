...
sample_t * dolby_atmos_512_effect_run(struct effect *e, ssize_t *frames, sample_t *ibuf, sample_t *obuf)
{
	...
	for (i = 0; i < *frames; ++i) {
		...
		sample_t ll_m = 0, lr_m = 0, rl_m = 0, rr_m = 0;
		sample_t lsl_m = 0, lsr_m = 0, rsl_m = 0, rsr_m = 0;
		sample_t lhl_m = 0, lhr_m = 0, rhl_m = 0, rhr_m = 0;
		...
		*/
		...
		/* Coefficients for height channels (LH and RH) */
		const double height_gain = 0.7;  /* Adjust height gain to taste */
		lhl_m = ll_m * height_gain;
		lhr_m = lr_m * height_gain;
		rhl_m = rl_m * height_gain;
		rhr_m = rr_m * height_gain;
		...
		if (state->has_output) {
			...
			for (k = 0; k < e->istream.channels; ++k) {
				...
				obuf[oframes*e->ostream.channels + k + 2] = 0.0;
				obuf[oframes*e->ostream.channels + k + 3] = 0.0;
				++oframes;
			#endif
		}
...
struct effect * dolby_atmos_512_effect_init(struct effect_info *ei, struct stream_info *istream, char *channel_selector, const char *dir, int argc, char **argv)
{
	...
	if (argc > 3)
		opt = argv[1];
	if (argc > 2) {
		surr_mult = pow(10.0, strtod(argv[argc-2], &endptr) / 20.0);
		CHECK_ENDPTR(argv[argc-2], endptr, "surround_level", return NULL);
		if (surr_mult > 1.0)
			LOG_FMT(LL_ERROR, "%s: warning: surround_level probably shouldn't be greater than 0", argv[0]);
	}
	if (argc > 1) {
		height_mult = pow(10.0, strtod(argv[argc-1], &endptr) / 20.0);
		CHECK_ENDPTR(argv[argc-1], endptr, "height_level", return NULL);
		if (height_mult > 1.0)
			LOG_FMT(LL_ERROR, "%s: warning: height_level probably shouldn't be greater than 0", argv[0]);
	}
	...
	e->ostream.channels = istream->channels + 6;  /* Adding 6 channels for LS, RS, LH, RH */
	...
	state->norm_mult = 1.0 / sqrt(1.0 + surr_mult*surr_mult + height_mult*height_mult);
